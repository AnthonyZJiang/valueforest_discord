...
class HandshakeInitiator:
    ...
    def is_valid_handshake_message(self, message: discord.Message) -> bool:
        return message.channel.id == self.channel_id and message.content.startswith(self.responder_name)
...
class HandshakeResponder:
    ...
    def __init__(self, client: Selfbot):
        ...
        self.channel_id = client.config.keepalive.handshake_channel_id
        self.interval = client.config.keepalive.handshake_interval * 0.99 # allow a small buffer to avoid timestamp mismatch
    ...
    def is_valid_handshake_message(self, message: discord.Message) -> bool:
        return message.channel.id == self.channel_id and message.content.startswith(self.initiator_name)
