...
class KeepAlive:
    ...
    async def _spin_once(self) -> None:
        if (
            self._pull_only_action_layer.selfbot_ready
            or self._pull_only_action_layer.selfbot_start_timeout
        ):
        ...
        if self._monitor_layer.handshake_timeout:
            ...
            self._action_layer.restart_bot()
            if datetime.now() - self._monitor_layer.last_ok_datetime > timedelta(
                seconds=self._client.config.keepalive.down_time_require_pull_only_seconds
            ):
